...
function generateHeartbeats(): object[] {
  ...
  for (let i = 0; i < 72; i++) {
    ...
    // rain/soil carry `since` = when they last changed. For seed data we anchor the rain
    // streak (hours 18–22 ago) to its onset so the rain-pause window has a realistic start;
    // soil simply marks this heartbeat's own time.
