...
const AIScheduleConfigModal = ({ open, onClose, onSaved, inline = false, zones = [], refreshKey }: AIScheduleConfigModalProps) => {
  const configQuery = useQuery({
    queryKey: ["aiScheduleModalConfig", refreshKey],
    queryFn: async () => (await fetchAIScheduleConfig()) ?? null,
    enabled: open
  });

  const lastRunQuery = useQuery({
    queryKey: ["aiScheduleModalLastRun", refreshKey],
  ...
  const loading = configQuery.isLoading || lastRunQuery.isLoading;
  const config = configQuery.data ?? null;
  const lastRun = lastRunQuery.data?.run ?? null;
  const lastRunEntries = lastRunQuery.data?.entries ?? [];
...
interface AIScheduleFormProps {
  initialConfig: AIScheduleConfig | null;
...
const AIScheduleForm = ({ initialConfig, lastRun, lastRunEntries, inline, onClose, onSaved, zones }: AIScheduleFormProps) => {
  ...
  const initialParsedCron = parseCron(initialConfig?.scheduleCron ?? "");

  const [enabled, setEnabled] = useState(initialConfig?.enabled ?? false);
  const [provider, setProvider] = useState<"anthropic" | "openai" | "google">(initialConfig?.provider ?? "anthropic");
  ...
  const [evalWindow, setEvalWindow] = useState(initialConfig?.evaluationWindowHours ?? 24);
  const [userContext, setUserContext] = useState(initialConfig?.userContext ?? "");
  const [prefs, setPrefs] = useState<AISchedulePreferences>(initialConfig?.preferences ?? DEFAULT_PREFS);
