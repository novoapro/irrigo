...
const IrrigationSettingsTab = ({ zones, onScheduleChanged, aiRunRefreshKey }: IrrigationSettingsTabProps) => {
  ...
  const [rainPauseHours, setRainPauseHours] = useState(48);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(async ([irrigSettings, aiConfig, runsResult]) => {
        if (cancelled) return;

        setTimeWindows(irrigSettings.preferredTimeWindows);
        setWaterSavingMode(irrigSettings.waterSavingMode);
        setRainPauseHours(irrigSettings.rainPauseHours ?? 48);
        setTimezone(irrigSettings.timezone ?? "America/New_York");
        ...
        if (aiConfig) {
          ...
          const parsed = parseCron(aiConfig.scheduleCron);
          setScheduleFrequency(parsed.frequency);
          setSelectedDays(parsed.selectedDays);
          setScheduleHour(parsed.hour);
          setEvalWindow(aiConfig.evaluationWindowHours);
          setUserContext(aiConfig.userContext);
          setPrefs(aiConfig.preferences ?? DEFAULT_PREFS);
        }
        ...
        if (runs && runs.length > 0) {
          ...
          try {
            ...
            if (!cancelled) setLastRunEntries(Array.isArray(detail.entries) ? detail.entries : []);
          } catch { /* ignore */ }
        }
